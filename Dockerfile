...
RUN apt-get update -y && apt-get --no-install-recommends install -y build-essential git \
...
RUN apt-get update -y && apt-get --no-install-recommends install -y libstdc++6 openssl libncurses5 locales curl \
  && apt-get clean && rm -f /var/lib/apt/lists/*_*
...
COPY --from=builder /app/_build/${MIX_ENV}/rel/integration_reports ./
COPY entrypoint.sh .

RUN chown nobody /app
RUN chown -R nobody: /app
RUN chown -R nobody: entrypoint.sh
